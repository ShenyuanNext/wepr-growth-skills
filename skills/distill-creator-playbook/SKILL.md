...
1. Define the question the analysis must answer: positioning, topic selection, packaging, retention, conversion, or operating cadence.
2. Build a traceable sample inventory with URL or identifier, date, format, title, topic, observable engagement, and evidence status.
3. Separate observation from inference. Mark missing, inaccessible, deleted, or login-gated material.
4. Code each item using the same dimensions: audience, job-to-be-done, hook, promise, structure, proof, emotion, visual device, CTA, and comment signal.
5. Compare high-, middle-, and low-performing samples when comparable data exists. Do not infer causation from engagement alone.
6. Identify recurring choices, controlled variations, exceptions, and possible confounders such as timing, paid distribution, celebrity, or prior audience size.
7. Distill principles at three levels: stable positioning, reusable content modules, and testable packaging tactics.
8. Translate principles into an original playbook with topic lanes, templates, examples, quality checks, and a 2–4 week test plan.
...
- Read [references/coding-framework.md](references/coding-framework.md) when analyzing samples.
- Read [references/playbook-output.md](references/playbook-output.md) when producing the final playbook or account comparison.
