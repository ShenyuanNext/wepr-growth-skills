...
chmod 600 "$wepr-geoflow_login_request" "$wepr-geoflow_login_response"
trap 'rm -f "$wepr-geoflow_login_request" "$wepr-geoflow_login_response" "${geoflow_auth_header:-}"' EXIT
python3 - "$wepr-geoflow_login_request" <<'PY'
import getpass
import json
import pathlib
import sys
...
PY
curl --disable --proto "$wepr-geoflow_curl_proto" -sS --fail-with-body --max-time 20 --max-filesize 5242880 -X POST \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
  --data-binary "@$wepr-geoflow_login_request" \
  --output "$wepr-geoflow_login_response" \
  "$GEOFLOW_BASE_URL/api/v1/auth/login"
GEOFLOW_API_TOKEN="$(python3 - "$wepr-geoflow_login_response" <<'PY'
import json
import pathlib
import sys
...
chmod 600 "$wepr-geoflow_auth_header"
printf 'Authorization: Bearer %s\n' "$GEOFLOW_API_TOKEN" > "$wepr-geoflow_auth_header"
...
```bash
curl --disable --proto "$wepr-geoflow_curl_proto" -sS --fail-with-body --max-time 20 --max-filesize 5242880 \
  --header "@$wepr-geoflow_auth_header" \
  -H "Accept: application/json" \
...
curl --disable --proto "$wepr-geoflow_curl_proto" -sS --fail-with-body --max-time 20 --max-filesize 5242880 -X PATCH \
  --header "@$wepr-geoflow_auth_header" \
  -H "Accept: application/json" \
  -H "Content-Type: application/json" \
...
curl --disable --proto "$wepr-geoflow_curl_proto" -sS --fail-with-body --max-time 20 --max-filesize 5242880 -X DELETE \
  --header "@$wepr-geoflow_auth_header" \
  -H "Accept: application/json" \
