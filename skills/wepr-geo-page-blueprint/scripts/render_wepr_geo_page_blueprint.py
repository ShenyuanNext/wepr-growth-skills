...
def slugify(value: str) -> str:
    return re.sub(r"[^a-z0-9]+", "-", value.lower()).strip("-") or "wepr-geo-page-blueprint"
...
def render_docx(data: dict[str, Any], path: Path) -> None:
    ...
    core = f'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>{xe(data["title"])}</dc:title><dc:creator>{xe(data.get("prepared_by", "wepr-geo-page-blueprint"))}</dc:creator></cp:coreProperties>'
...
def review(paths: dict[str, Path]) -> dict[str, Any]:
    ...
    return {
        "skill_id": "wepr-geo-page-blueprint",
