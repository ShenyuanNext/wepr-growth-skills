...
def render_docx_xml(data: dict[str, Any]) -> str:
    p = data["project"]
    body = [wp(data["report_title"], "Title", 180), wp(data.get("subtitle", ""), "Subtitle", 220), wp(f"生成日期：{data['generated_at']}    生成器：{data.get('prepared_by','wepr-geo-title-optimizer')}", "Meta", 240)]
...
def review(data: dict[str, Any], paths: dict[str, Path], html_text: str) -> dict[str, Any]:
    ...
    return {"skill_id": "wepr-geo-title-optimizer", "checked_at": dt.datetime.now().isoformat(timespec="seconds"), "files": {k: {"path": str(p), "exists": p.exists(), "size": p.stat().st_size if p.exists() else 0} for k, p in paths.items()}, "report_depth_checks": depth, "docx_layout_checks": docx, "pdf_checks": pdf, "issues": issues, "passed": not any(x["severity"] in {"blocker", "major"} for x in issues)}
