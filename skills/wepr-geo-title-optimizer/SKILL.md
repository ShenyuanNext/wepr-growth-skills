...
- Optional real-data inputs: public URLs, local customer documents, exported platform answers, CMS fields, and evidence snapshots produced by `scripts/collect_wepr_geo_title_evidence.py`.
...
The renderer expects a completed report JSON, not a raw keyword brief. Build the report JSON from the user's brief before running `scripts/render_wepr_geo_title_optimizer.py`.
...
```bash
python3 scripts/collect_wepr_geo_title_evidence.py --input examples/<case>/report_input.json --output examples/<case>/evidence_snapshot.json
```
