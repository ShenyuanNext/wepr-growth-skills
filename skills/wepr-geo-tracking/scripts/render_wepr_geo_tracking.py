...
def slugify(value: str) -> str:
    ...
    return lowered.strip("-") or "wepr-geo-tracking"
...
def render_html(data: dict[str, Any]) -> str:
    ...
    prepared_by = html.escape(data.get("prepared_by", "wepr-geo-tracking"))
...
<body>
  <main class="page">
    <header class="hero">
      <p class="eyebrow">WEPR GEO Tracking</p>
      <h1>{title}</h1>
...
def render_docx(data: dict[str, Any], output_path: Path) -> None:
    ...
    meta = (
        ...
        f"生成方式：{data.get('prepared_by', 'wepr-geo-tracking')}"
    )
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties"
 ...
 xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <dc:creator>{xml_escape(data.get("prepared_by", "wepr-geo-tracking"))}</dc:creator>
  <cp:lastModifiedBy>{xml_escape(data.get("prepared_by", "wepr-geo-tracking"))}</cp:lastModifiedBy>
