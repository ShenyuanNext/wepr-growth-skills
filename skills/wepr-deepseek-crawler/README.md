...
`wepr-deepseek-crawler` 是一个 DeepSeek 网页端 AI 搜索重复采样与 GEO 概率分析 Skill。它会借助本地 OpenCLI Browser Bridge 和已登录的 DeepSeek 网页会话，对用户给定的一组关键词进行多轮独立采样，保存原始证据，并生成结构化 Markdown、Excel 和 HTML 可视化诊断报告。
