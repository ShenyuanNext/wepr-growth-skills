...
def load_data(path: Path) -> dict[str, Any]:
    ...
    data.setdefault("skill_id", "wepr-geo-explainer-builder")
...
""".strip()
    return f'<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>{html.escape(data["title"])}</title><style>{css}</style></head><body><main class="page"><header class="cover"><p class="eyebrow">WEPR GEO Explainer Builder</p><h1>{html.escape(data["title"])}</h1><div class="meta">{meta}</div></header>{nav}{"".join(section_html)}</main></body></html>'
