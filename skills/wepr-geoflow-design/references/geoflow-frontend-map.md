...
This document is the current GEOFlow frontend contract for `wepr-geoflow-design`. It is based on the Laravel rewrite, not the older root-level PHP frontend.
...
Do not create lead forms from this design skill. If no active form slug is available, use a `cta_band` that links to the intended public form URL or hand off lead-form creation to `wepr-geoflow-cli` through admin web operations.
