...
Stage 1 writes `chatgpt-crawl.json`:

```json
{
  "schema_version": "wepr-chatgpt-crawler/v1",
...
- `result`, the normalized ChatGPT crawler JSON
...
- `reference_count`: total number of visible ChatGPT citation pills, source-flyout links, markdown links, or bare URLs extracted by the crawler.
- `unique_urls`: unique cited URLs.
- `unique_domains`: unique cited domains.

## Target Entity Recognition

Standard mode: user supplies these inputs:
...
- `profile`: the OpenCLI Browser Bridge profile.
- `single_query_interval`: delay preset between fresh samples; default random `30s-1m`, optional `1-3m` or `3-10m`.
...
Flatten every `references.items[]` row. Preserve `number`, `source`, `domain`, `title`, `date`, `url`, `summary`, `source_position`, `positions`, and `extraction_method` when present. ChatGPT may expose only a visible citation pill, source-flyout row, markdown link, or bare URL; missing reference fields are data availability limits, not proof that no source was used.
...
Domain rows should include a readable Simplified Chinese display name when it can be inferred from the ChatGPT source label or a maintained domain-name mapping.
