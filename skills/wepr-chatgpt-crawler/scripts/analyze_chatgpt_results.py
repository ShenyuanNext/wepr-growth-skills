...
TITLE_INTENT_LABELS_EN = {
    "榜单排名": "Ranking/List",
    "对比评测": "Comparison/Review",
    "推荐选择": "Recommendation/Choice",
    "避坑风险": "Risk Avoidance",
    "指南攻略": "Guide/How-to",
    "趋势新闻": "Trend/News",
    "资料介绍": "Profile/Intro",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze ChatGPT repeated crawl JSON and render a Kami-styled HTML report."
    )
    parser.add_argument("input_json", help="chatgpt-crawl.json, results-all.json, or a single ChatGPT raw JSON")
    ...
    parser.add_argument("--target-entity", help="Primary target entity, for example 新东方, a founder name, a product name, or a company name")
    ...
    parser.add_argument("--title", default="ChatGPT AI 搜索概率分析报告")
...
def normalize_ref(ref: dict) -> dict:
    ...
    raw_positions = ref.get("positions") or []
    if isinstance(raw_positions, str):
        positions = [clean_text(item) for item in re.split(r"[,+|/]", raw_positions) if clean_text(item)]
    elif isinstance(raw_positions, list):
        positions = [clean_text(item) for item in raw_positions if clean_text(item)]
    else:
        positions = []
    source_position = clean_text(ref.get("source_position") or ref.get("position"))
    if source_position and source_position not in positions:
        positions.insert(0, source_position)
    return {
        ...
        "source_position": "+".join(dict.fromkeys(positions)) if positions else source_position,
        "positions": list(dict.fromkeys(positions)),
        "extraction_method": clean_text(ref.get("extraction_method")),
    }


def normalize_samples(data: dict) -> list[dict]:
    samples: list[dict] = []
    if str(data.get("schema_version", "")).startswith("wepr-chatgpt-crawler/"):
    ...
    if data.get("engine") == "chatgpt" or data.get("answer"):
...
def call_openai_semantic_review(contexts: list[dict]) -> dict[str, dict]:
    ...
    system_prompt = (
        "You classify candidate entities for a ChatGPT GEO report. "
...
def load_brand_defs(
    ...
    if target_profile["has_target"]:
        ...
        inferred_alias_candidates = [] if has_provided_rows else entity_candidates
        target_aliases = merge_target_aliases(target_profile["aliases"], inferred_alias_candidates, rows)
...
def dedupe_brand_rows(rows: list[list[str]]) -> list[dict]:
    ...
    for row in rows:
        aliases = []
        for index, alias in enumerate(row):
            if not alias:
                continue
            if index > 0 and looks_like_stop_entity(alias):
                continue
...
def classify_entity(row: dict, target_kind: str) -> dict:
    ...
    confidence = 0.35
    why: list[str] = []
    if reasons.get("provided_or_metric_entity") and target_kind in {"person", "company", "product"}:
        kind = target_kind
        confidence = 0.9
        why.append("用户提供或进入指标计算的实体")
    elif looks_like_noise_entity(name):
        kind = "noise"
        confidence = 0.1
...
def build_entity_candidates(
    ...
    for brand in brands:
        ...
        row["reasons"]["provided_or_metric_entity"] = max(row["reasons"].get("provided_or_metric_entity", 0), 1)
...
def structured_export_tables(summary: dict, output_files: dict[str, str]) -> list[dict]:
    ...
    overview_rows = [
        ...
        {"field": "reference_count", "value": samples.get("reference_count"), "description": "ChatGPT 引用信源总数"},
    ...
    return [
        ...
        export_table("信源渠道分布", channel_rows, ["bucket", "raw_bucket", "count", "rate"], "ChatGPT 引用信源的渠道结构。"),
        export_table("来源编号位置", position_rows, ["bucket", "raw_bucket", "count", "rate"], "ChatGPT 引用编号或链接位置分布。"),
...
def lang_pair(zh: object, en: object) -> str:
    return f'<span class="lang-zh">{esc(zh)}</span><span class="lang-en">{esc(en)}</span>'
...
def render_benchmark_radar(selected_rows: list[dict], benchmark_rows: list[tuple[str, dict]], lang: str = "zh") -> str:
    selected = selected_rows[:4]
    if not selected:
        return '<p class="empty">No data.</p>'
    specs = [
        ("mention_rate", "Mention" if lang == "en" else "提及"),
        ("avg_mentions_per_sample", "Intensity" if lang == "en" else "提及强度"),
        ("top1_rate", "Top 1"),
        ("top3_rate", "Top 3"),
        ("top5_rate", "Top 5"),
        ("reference_mentions", "Citations" if lang == "en" else "信源"),
    ]
    ...
    return (
        ...
        f'<svg class="viz-svg radar benchmark-radar" viewBox="0 0 420 320" role="img" aria-label="{esc("Target vs competitors radar" if lang == "en" else "目标与竞品 100 分制雷达对标")}">'
        f'<g class="radar-grid">{"".join(rings)}{"".join(axes)}</g>'
        f'{"".join(series)}'
        f'<g class="radar-labels">{"".join(labels)}</g>'
        f'<text class="axis-label" x="300" y="292">{esc("Best in set = 100" if lang == "en" else "行业最优 = 100 分")}</text>'
        '</svg>'
...
def render_benchmark_bubble_chart(rows: list[tuple[str, dict]], target_name: str, limit: int, lang: str = "zh") -> str:
    selected = rows[:limit]
    target_entry = next(
    ...
    for index, (name, metric) in enumerate(selected):
        ...
        if lang == "en":
            aria = (
                ...
                f"mention rate {pct(metric.get('mention_rate') or 0)}, average mentions {fmt_num(metric.get('avg_mentions_per_sample') or 0, 2)}"
            )
        else:
            aria = (
                f"{name}，Top 5 {pct(metric.get('top5_rate') or 0)}，"
    ...
    return (
        ...
        f'<svg class="viz-svg bubble-plot" viewBox="0 0 {width} {height}" role="img" aria-label="{esc("Top 5 probability and mention-rate bubble benchmark" if lang == "en" else "Top 5 概率与提及率气泡对标")}">'
        '<rect class="bubble-zone" x="62" y="28" width="424" height="248" />'
        f'{"".join(grid)}'
        f'<line class="axis" x1="{left}" y1="{height-bottom}" x2="{width-right}" y2="{height-bottom}" />'
        f'<line class="axis" x1="{left}" y1="{top}" x2="{left}" y2="{height-bottom}" />'
        f'<text class="axis-label" x="274" y="320" text-anchor="middle">{esc("Top 5 Probability" if lang == "en" else "Top 5 概率")}</text>'
        f'<text class="axis-label" x="18" y="155" transform="rotate(-90 18 155)" text-anchor="middle">{esc("Mention Rate" if lang == "en" else "提及率")}</text>'
        f'<text class="quadrant-label" x="382" y="48">{esc("High coverage / high mention" if lang == "en" else "高覆盖 / 高提及")}</text>'
        f'{"".join(nodes)}'
        '</svg>'
...
def render_metric_matrix(rows: list[tuple[str, dict]], limit: int, lang: str = "zh") -> str:
    headers = [
        ("mention_rate", "Mention" if lang == "en" else "提及"),
        ("avg_mentions_per_sample", "Avg" if lang == "en" else "均次"),
        ("top1_rate", "Top1"),
        ("top3_rate", "Top3"),
        ("top5_rate", "Top5"),
    ]
    ...
    first_header = "Entity" if lang == "en" else "实体"
    return f'<table class="kami-table matrix-table"><thead><tr><th>{esc(first_header)}</th>{header}</tr></thead><tbody>{"".join(body)}</tbody></table>'
...
def render_coverage_snapshot(samples: dict, question_rows: list[dict], lang: str = "zh") -> str:
    if lang == "en":
        segments = [
            ("Valid", samples.get("valid", 0), "seg-positive"),
            ("Failed", samples.get("failed", 0), "seg-negative"),
            ("Pending", samples.get("pending", 0), "seg-neutral"),
        ]
        cards = [
            ("Completion", pct(samples.get("completion_rate") or 0)),
            ("Valid Rate", pct(samples.get("valid_rate") or 0)),
            ("Avg Citations", fmt_num(sum(row.get("avg_references", 0) for row in question_rows) / len(question_rows) if question_rows else 0)),
            ("Avg Chars", fmt_num(sum(row.get("avg_answer_chars", 0) for row in question_rows) / len(question_rows) if question_rows else 0, 0)),
        ]
        title = "Sample Status Mix"
    else:
        segments = [
...
def feature_rate_rows(features: dict, lang: str = "zh") -> list[tuple[str, float, str]]:
    labels = {
        "has_number": "Has Number" if lang == "en" else "标题含数字",
        "has_year": "Has Year" if lang == "en" else "标题含年份",
        "has_question": "Question Form" if lang == "en" else "标题含疑问",
        "has_structure_punctuation": "Structured Punctuation" if lang == "en" else "标题含结构符",
        "contains_brand": "Contains Entity" if lang == "en" else "标题含实体",
    }
...
def title_intent_rows(features: dict, limit: int = REPORT_ITEM_LIMIT, lang: str = "zh") -> list[tuple[str, float, str]]:
    total = max(features.get("total") or 0, 1)
    counts = Counter(features.get("intent_counts") or {})
    return [
        (TITLE_INTENT_LABELS_EN.get(label, label) if lang == "en" else label, count / total, f"{pct(count / total)} · {count}")
...
def report_overview_text(summary: dict) -> str:
    ...
    return (
        f"本报告基于 {samples['question_count']} 个关键词、{samples['planned']} 次计划采样、{samples['valid']} 条有效 ChatGPT AI 搜索结果，"
...
def report_overview_text_en(summary: dict) -> str:
    ...
    return (
        ...
        f"and {samples['valid']} valid ChatGPT AI search answers. It evaluates the target entity \"{entity_name}\" "
...
def metric_definition_html(lang: str = "zh") -> str:
    if lang == "en":
        rows = [
            ...
            ("Citation", "Source title, domain, channel, and URL cited by ChatGPT."),
        ]
    else:
        rows = [
            ("提及率", "实体在有效回答中被提到的比例。"),
...
def semantic_status_label(value: str, lang: str = "zh") -> str:
    ...
    return labels.get(lang, labels["zh"]).get(value, value or ("Not run" if lang == "en" else "未执行"))


def semantic_label(value: str, lang: str = "zh") -> str:
    labels = {
        "zh": {
            "target_alias": "目标别名",
        ...
        "en": {
            ...
            "attribute": "Attribute",
            "service_or_feature": "Service/Feature",
            "source_or_title": "Source/Title",
            "unrelated_entity": "Unrelated Entity",
            "uncertain": "Needs Review",
        },
    }
    return labels.get(lang, labels["zh"]).get(value, value or ("Needs Review" if lang == "en" else "待确认"))


def entity_kind_display(value: str, lang: str = "zh") -> str:
    labels = {
        "zh": {"person": "人", "company": "公司", "product": "产品", "mixed": "混合实体", "concept": "概念"},
        "en": {"person": "Person", "company": "Company", "product": "Product", "mixed": "Mixed", "concept": "Concept"},
    }
    return labels.get(lang, labels["zh"]).get(value, value or ("Mixed" if lang == "en" else "混合实体"))


def yes_no(value: object, lang: str = "zh") -> str:
    return ("Yes" if value else "No") if lang == "en" else ("是" if value else "否")


def entity_reason_en(row: dict) -> str:
    label = row.get("semantic_label")
    if label == "target_alias" or row.get("role") == "target":
        return "The candidate matches the target entity or one of its aliases."
    if label == "direct_competitor" or row.get("entered_competitor_matrix"):
        return "The candidate is a same-type entity with answer-body evidence."
    if label == "generic_category":
        return "The candidate is an industry, category, or selection phrase, not a concrete entity name."
    if label == "attribute":
        return "The candidate is an attribute, credential, region, or generic descriptor rather than an entity name."
    if label == "service_or_feature":
        return "The candidate looks like a service capability or feature, not an independent competitor entity."
    if label == "source_or_title":
        return "The candidate lacks answer-body evidence and mainly appears in source titles or citation metadata."
    if label == "unrelated_entity" or row.get("semantic_is_same_type") is False:
        return "The candidate does not match the target entity type."
    return "The candidate has insufficient evidence and should be manually reviewed."


def render_report(title: str, input_name: str, summary: dict) -> str:
    default_title_zh = "ChatGPT AI 搜索概率分析报告"
    title_en = "ChatGPT AI Search Probability Report" if clean_text(title) == default_title_zh else title
    samples = summary["samples"]
    item_limit = int((summary.get("report") or {}).get("item_limit") or REPORT_ITEM_LIMIT)
    target = summary.get("target", {})
    target_metrics = target.get("metrics") or {}
    semantic_review = summary.get("semantic_review") or {}
    semantic_status_text = semantic_status_label(semantic_review.get("status"), "zh")
    semantic_status_text_en = semantic_status_label(semantic_review.get("status"), "en")
    brand_source_note = {
        "provided": "用户提供实体表",
    ...
    brand_source_note_en = {
        "provided": "User-provided entity list",
        "target_entity": "Same-type competitors identified around the target entity",
        "inferred_target_entity": "Target entity inferred automatically",
        "inferred": "Candidate entities inferred automatically",
    }.get(summary["brand_source"], summary["brand_source"])
    entity_kind_label = entity_kind_display(summary.get("entities", {}).get("target_kind"), "zh")
    entity_kind_label_en = entity_kind_display(summary.get("entities", {}).get("target_kind"), "en")
    top1_rows = [
    ...
    negative_rows_en = [
        (name, row["negative_rate"], f"{pct(row['negative_rate'])} · {sentiment_label_en(row.get('dominant_sentiment') or 'neutral')}")
        for name, row in sorted_brand_rows(summary, "negative_rate")[:item_limit]
    ]
    ...
    recency_rows_en = [
        (RECENCY_LABELS_EN.get(name, name), count / recency_total, f"{pct(count / recency_total)} · {count}")
        for name, count in summary["titles"]["recency_buckets"].items()
    ]
    url_link_rows = [
        {
    ...
    sentiment_segments_en = [
        ("Positive", int(target_sentiment_counts.get("positive", 0)), "seg-positive"),
        ("Neutral", int(target_sentiment_counts.get("neutral", 0)), "seg-neutral"),
        ("Negative", int(target_sentiment_counts.get("negative", 0)), "seg-negative"),
    ]
    ...
    for row in comparison_rows:
        is_target_row = row.get("role") == "target"
        comparison_table_parts.append(
            f"<tr><td>{lang_pair('目标' if is_target_row else '竞品', 'Target' if is_target_row else 'Competitor')}</td><td>{esc(row['name'])}</td>"
            ...
            f"<td>{row.get('reference_mentions', 0)}</td><td>{lang_pair('基准', 'Baseline') if is_target_row else pp(row.get('gap_vs_target_top3'))}</td>"
            f"<td>{lang_pair('基准', 'Baseline') if is_target_row else pp(row.get('gap_vs_target_top5'))}</td></tr>"
        )
    comparison_table = "".join(comparison_table_parts)

    entity_table_parts = []
    for row in summary.get("entities", {}).get("candidates", [])[:item_limit]:
        reason_zh = row.get("semantic_reason") or "；".join(row.get("reasons") or ["候选项"])
        entity_table_parts.append(
            f"<tr><td>{esc(row['name'])}</td><td>{esc(row['kind'])}</td>"
            f"<td>{lang_pair(semantic_label(row.get('semantic_label'), 'zh'), semantic_label(row.get('semantic_label'), 'en'))}</td>"
            f"<td>{lang_pair(yes_no(row.get('semantic_is_same_type'), 'zh'), yes_no(row.get('semantic_is_same_type'), 'en'))}</td>"
            f"<td>{lang_pair(yes_no(row.get('entered_competitor_matrix'), 'zh'), yes_no(row.get('entered_competitor_matrix'), 'en'))}</td>"
            ...
            f"<td>{lang_pair(reason_zh, entity_reason_en(row))}</td></tr>"
        )
    entity_table = "".join(entity_table_parts)
    title_table = "".join(
...
<main>
  <nav class="report-nav" aria-label="报告目录">
    <div class="report-nav-shell">
      <a class="report-brand" href="#top" aria-label="回到报告顶部">
        <span class="report-brand-mark">ChatGPT GEO</span>
  ...
  <section class="cover" id="top">
    <div class="eyebrow">CHATGPT GEO CRAWL REPORT</div>
    <h1><span class="lang-zh">{esc(title)}</span><span class="lang-en">{esc(title_en)}</span></h1>
	    <p class="lede lang-zh">基于重复采样的 ChatGPT 网页端 AI 搜索结果，识别真正的目标人物或品牌/机构，再衡量提及、平均提及、Top 1 / Top 3 / Top 5 概率、情感倾向，以及信源与标题结构。<br><span class="entity-scope">实体口径：<span class="tag">{esc(brand_source_note)}</span></span></p>
	    <p class="lede lang-en">Based on repeated ChatGPT web AI search samples, this report identifies the real target person, brand, or organization, then measures mentions, average mentions, Top 1 / Top 3 / Top 5 probability, sentiment, citations, and title patterns.</p>
  </section>
  ...
  <section class="metric-grid lang-zh">
  ...
  <section class="metric-grid lang-en">
    {metric_card(samples['question_count'], 'Questions', input_name)}
    {metric_card(samples['planned'], 'Planned Samples', f"completed {samples['completed']}, pending {samples['pending']}")}
    {metric_card(samples['valid'], 'Valid Samples', f"failed {samples['failed']}, valid rate {pct(samples['valid_rate'])}")}
    {metric_card(samples['reference_count'], 'Citations', f"{samples['unique_domains']} domains · {samples['unique_urls']} URLs")}
    {metric_card(samples['answer_chars'], 'Answer Chars', f"completion {pct(samples['completion_rate'])}")}
  </section>
  ...
  <section class="section" id="comparison">
    ...
    <div class="chart-grid">
      ...
      <div class="chart"><h3><span class="lang-zh">Top 5 概率 x 提及率气泡对标</span><span class="lang-en">Top 5 Probability x Mention Rate Bubble Benchmark</span></h3><div class="lang-zh">{render_benchmark_bubble_chart(metric_rows, target.get("entity") or "", item_limit, 'zh')}</div><div class="lang-en">{render_benchmark_bubble_chart(metric_rows, target.get("entity") or "", item_limit, 'en')}</div></div>
    </div>
    <h3><span class="lang-zh">同类型实体多指标矩阵</span><span class="lang-en">Same-Type Entity Metric Matrix</span></h3>
    <div class="lang-zh">{render_metric_matrix(metric_rows, item_limit, 'zh')}</div>
    <div class="lang-en">{render_metric_matrix(metric_rows, item_limit, 'en')}</div>
    <table class="kami-table">
      <thead><tr><th>{lang_pair('角色', 'Role')}</th><th>{lang_pair('实体', 'Entity')}</th><th>{lang_pair('提及率', 'Mention Rate')}</th><th>{lang_pair('平均提及', 'Avg Mentions')}</th><th>Top 1</th><th>Top 3</th><th>Top 5</th><th>{lang_pair('平均排名', 'Avg Rank')}</th><th>{lang_pair('信源命中', 'Citation Hits')}</th><th>{lang_pair('Top 3 差距', 'Top 3 Gap')}</th><th>{lang_pair('Top 5 差距', 'Top 5 Gap')}</th></tr></thead>
  ...
  <section class="section" id="sentiment">
    ...
    <div class="chart-grid three">
      <div class="chart"><h3><span class="lang-zh">目标实体情感分布</span><span class="lang-en">Target Sentiment Mix</span></h3><div class="lang-zh">{render_sentiment_donut(sentiment_segments, sentiment_total)}</div><div class="lang-en">{render_sentiment_donut(sentiment_segments_en, sentiment_total)}</div></div>
      ...
      <div class="chart"><h3><span class="lang-zh">负向占比</span><span class="lang-en">Negative Share</span></h3><div class="lang-zh">{bar_rows(negative_rows, 1)}</div><div class="lang-en">{bar_rows(negative_rows_en, 1)}</div></div>
    </div>
  </section>

	  <section class="section" id="entities">
	    <h2><span class="lang-zh">目标实体识别</span><span class="lang-en">Entity Identification</span></h2>
	    <p class="entity-note lang-zh">目标类型：{esc(entity_kind_label)}。语义复核状态：{esc(semantic_status_text)}。系统会把人、公司、产品、概念词和噪声词分开；只有同类型候选项会进入目标与竞品概率计算。自动识别是启发式判断，正式报告建议提供实体别名表并开启 required 语义复核。</p>
	    <p class="entity-note lang-en">Target type: {esc(entity_kind_label_en)}. Semantic review status: {esc(semantic_status_text_en)}. The system separates people, companies, products, concepts, and noise terms; only same-type candidates enter target and competitor probability calculations. Automatic recognition is heuristic; formal reports should provide an entity alias list and use required semantic review.</p>
	    <table class="kami-table">
	      <thead><tr><th>{lang_pair('候选项', 'Candidate')}</th><th>{lang_pair('规则类型', 'Rule Type')}</th><th>{lang_pair('语义标签', 'Semantic Label')}</th><th>{lang_pair('同类型', 'Same Type')}</th><th>{lang_pair('进入竞品', 'In Matrix')}</th><th>{lang_pair('语义置信度', 'Semantic Confidence')}</th><th>{lang_pair('样本数', 'Samples')}</th><th>{lang_pair('提及率', 'Mention Rate')}</th><th>Top 3</th><th>Top 5</th><th>{lang_pair('判定理由', 'Decision Reason')}</th></tr></thead>
  ...
  <section class="section" id="coverage">
    ...
    <div class="coverage-layout">
      <div class="chart">
        <h3>{lang_pair('每个问题的有效采样率', 'Valid Sample Rate by Question')}</h3>
        {bar_rows(question_chart_rows, 1)}
      </div>
      <div class="chart"><div class="lang-zh">{render_coverage_snapshot(samples, display_question_rows, 'zh')}</div><div class="lang-en">{render_coverage_snapshot(samples, display_question_rows, 'en')}</div></div>
    </div>
    <table class="kami-table">
      <thead><tr><th>{lang_pair('问题', 'Question')}</th><th>{lang_pair('问句', 'Prompt')}</th><th>{lang_pair('完成/计划', 'Done/Planned')}</th><th>{lang_pair('有效', 'Valid')}</th><th>{lang_pair('失败', 'Failed')}</th><th>{lang_pair('未完成', 'Pending')}</th><th>{lang_pair('平均信源', 'Avg Citations')}</th><th>{lang_pair('平均回答字数', 'Avg Answer Chars')}</th></tr></thead>
  ...
  <section class="section" id="probability">
    ...
    <div class="chart-grid three">
      <div class="chart"><h3>{lang_pair('Top 1 概率', 'Top 1 Probability')}</h3>{bar_rows(top1_rows, 1)}</div>
      <div class="chart"><h3>{lang_pair('Top 3 概率', 'Top 3 Probability')}</h3>{bar_rows(top3_rows, 1)}</div>
      <div class="chart"><h3>{lang_pair('Top 5 概率', 'Top 5 Probability')}</h3>{bar_rows(top5_rows, 1)}</div>
      <div class="chart"><h3>{lang_pair('提及率', 'Mention Rate')}</h3>{bar_rows(mention_rows, 1)}</div>
      <div class="chart"><h3>{lang_pair('平均提及次数', 'Average Mentions')}</h3>{bar_rows(avg_mention_rows, None)}</div>
      <div class="chart"><h3>{lang_pair('平均排名 · 数值越低越好', 'Average Rank · Lower Is Better')}</h3>{bar_rows(rank_rows, None)}</div>
    </div>
	    <h3>{lang_pair('目标实体明细', 'Target Entity Details')}</h3>
	    <table class="kami-table target-detail-table">
	      <thead><tr><th>{lang_pair('实体', 'Entity')}</th><th>{lang_pair('提及率', 'Mention Rate')}</th><th>{lang_pair('平均提及', 'Avg Mentions')}</th><th>Top 1</th><th>Top 3</th><th>Top 5</th><th>{lang_pair('平均排名', 'Avg Rank')}</th><th>{lang_pair('别名', 'Aliases')}</th></tr></thead>
	      <tbody>{brand_table}</tbody>
	    </table>
	    <h3>{lang_pair('问题 x 目标实体 Top 3 热力图', 'Question x Target Entity Top 3 Heatmap')}</h3>
  ...
  <section class="section" id="sources">
    ...
    {bilingual_summary(
        f"信源结构用于判断 ChatGPT 回答更依赖官方、媒体、社区、开发者或其他渠道；域名和来源明细默认只展示前 {item_limit} 条。",
        f"Source structure shows whether ChatGPT relies more on official, media, community, developer, or other channels. Domain and source details show up to {item_limit} rows.",
    )}
    <div class="chart-grid three">
      <div class="chart share-chart"><h3>{lang_pair('渠道分布', 'Channel Mix')}</h3><div class="lang-zh">{render_share_donut(channel_rows, CHANNEL_LABELS_ZH, "信源渠道分布圆环图")}</div><div class="lang-en">{render_share_donut(channel_rows, CHANNEL_LABELS_EN, "Citation channel mix donut")}</div></div>
      <div class="chart share-chart"><h3>{lang_pair('来源编号位置', 'Citation Position')}</h3><div class="lang-zh">{render_share_donut(position_rows, SOURCE_POSITION_LABELS_ZH, "来源编号位置圆环图")}</div><div class="lang-en">{render_share_donut(position_rows, SOURCE_POSITION_LABELS_EN, "Citation position donut")}</div></div>
      <div class="chart"><h3>{lang_pair('高频来源名', 'Frequent Source Names')}</h3>{bar_rows(source_rows, None)}</div>
      <div class="chart"><h3>{lang_pair('高频域名', 'Frequent Domains')}</h3>{source_bar_rows(domain_rows, domain_max)}</div>
      <div class="chart"><h3>{lang_pair('域名占比树图', 'Domain Share Treemap')}</h3>{render_treemap(summary["sources"]["top_domains"], item_limit)}</div>
      <div class="chart source-url-chart"><h3>{lang_pair('高频 URL', 'Frequent URLs')}</h3>{source_bar_rows(url_link_rows, None)}</div>
    </div>
  </section>
  ...
  <section class="section" id="titles">
    ...
    <div class="chart-grid">
      <div class="chart"><h3>{lang_pair('标题功能特征', 'Title Feature Signals')}</h3><div class="lang-zh">{bar_rows(feature_rate_rows(summary['titles'], 'zh'), 1)}</div><div class="lang-en">{bar_rows(feature_rate_rows(summary['titles'], 'en'), 1)}</div></div>
      <div class="chart"><h3>{lang_pair('标题长度', 'Title Length')}</h3>{bar_rows(length_rows, 1)}</div>
      <div class="chart"><h3>{lang_pair('时间新旧', 'Recency')}</h3><div class="lang-zh">{bar_rows(recency_rows, 1)}</div><div class="lang-en">{bar_rows(recency_rows_en, 1)}</div></div>
      <div class="chart"><h3>{lang_pair('标题意图特征分析', 'Title Intent Analysis')}</h3><div class="lang-zh">{bar_rows(title_intent_rows(summary['titles'], item_limit, 'zh'), 1)}</div><div class="lang-en">{bar_rows(title_intent_rows(summary['titles'], item_limit, 'en'), 1)}</div></div>
    </div>
    <h3>{lang_pair('高频引用标题', 'Frequent Cited Titles')}</h3>
    <table class="kami-table">
      <thead><tr><th>#</th><th>{lang_pair('标题', 'Title')}</th><th>{lang_pair('来源', 'Source')}</th><th>{lang_pair('次数', 'Count')}</th></tr></thead>
  ...
  <footer>
    Generated at {esc(summary['generated_at'])}. Metrics are repeated-sample estimates from ChatGPT web output, not external truth claims.
  </footer>
</main>
<script>
(function() {{
